...
def t_update(ctx, config, stop, module, device):
    while not stop.is_set():
        try:
            ...
            block_1601.add_16bit_int(config["ct_current"]) # ct rated current
            block_1601.add_16bit_int(config["p1_ct_current"]) # ct rated current l1
            block_1601.add_16bit_int(config["p2_ct_current"]) # ct rated current l2
            block_1601.add_16bit_int(config["p3_ct_current"]) # ct rated current l3
            block_1601.add_16bit_int(config["ct_inverted"]) # ct direction inversion
            ...
            block_1601.add_16bit_int(config["phase_offset"]) # phase offset
            ...
            ctx.setValues(3, 1650, block_1651.to_registers())

            block_1701 = BinaryPayloadBuilder(byteorder=Endian.Big, wordorder=Endian.Little)
            block_1701.add_32bit_int(config["serial_number"]) # serial number
            ...
            ctx.setValues(3, 1700, block_1701.to_registers())
        finally:
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    argparser.add_argument("-c", "--config", type=str, default="semp.conf")
    ...
    try:
        for meter in confparser["server"].get("meters", fallback=default_config["server"]["meters"]).split(','):
            ...
            meter_device = meter_module.device(
                ...
                parity=confparser[meter].get("parity", fallback=default_config["meters"]["parity"]),
            ...
            logger.debug(f"Meter {address} created: {meter_device}")
            ...
            update_t = threading.Thread(
                ...
                args=(
                    slave_ctx,
                    {
                        ...
                        "p1_ct_current": confparser[meter].getint("p1_ct_current", fallback=default_config["meters"]["p1_ct_current"]),
                        "p2_ct_current": confparser[meter].getint("p2_ct_current", fallback=default_config["meters"]["p2_ct_current"]),
                        "p3_ct_current": confparser[meter].getint("p3_ct_current", fallback=default_config["meters"]["p3_ct_current"]),
            ...
            logger.debug(f"Thread created {update_t}: {meter} {address}: {meter_type} {meter_device}")
        ...
        for t in threads:
            t.start()
            logger.debug(f"Started {t}")

        logger.debug(f"Starting ModbusRTU serial server on {device}")
        StartSerialServer(
            server_ctx,
